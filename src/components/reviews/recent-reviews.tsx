...
export function RecentReviews({
  ...
  const measureCardStep = useCallback(() => {
    ...
    const gap =
      Number.parseFloat(getComputedStyle(carousel).columnGap || "20") || 20;
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setLoadedReviews((current) => {
        ...
        return [
          ...current,
          ...page.reviews.filter((review) => !knownIds.has(review.id)),
        ];
      });
  ...
  const maybeLoadMore = useCallback(
    (nextStart: number) => {
      if (
        hasMore &&
        nextStart + VIRTUAL_WINDOW_SIZE >= loadedReviews.length - VIRTUAL_BUFFER
      ) {
        void loadMore();
      }
    },
  ...
  function reviewAt(index: number): ReviewDto | null {
    ...
    const normalizedIndex =
      ((index % loadedReviews.length) + loadedReviews.length) %
      loadedReviews.length;
  ...
  const renderedCount =
    loadedReviews.length > 1 ? VIRTUAL_WINDOW_SIZE : loadedReviews.length;

  return (
    <section
      aria-labelledby="recent-reviews-title"
      className="border-b border-line bg-white py-24 sm:py-32 lg:py-36"
    >
      <Container>
        <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <h2
              ...
              className="landing-display mt-5 max-w-[12ch] text-[clamp(3rem,6vw,6rem)] leading-[0.9] tracking-[-0.06em] text-ink"
            >
              What people say after using WrenPass.
            </h2>
            <p className="mt-6 max-w-xl text-base leading-7 text-ink-muted">
              Every review is submitted by a Stellar wallet and read directly
              from the public review contract.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            <Link
              className="ml-1 inline-flex h-11 items-center gap-2 rounded-lg px-4 text-sm font-bold text-forest transition hover:bg-mint-soft"
              href="/reviews"
            >
        ...
        {loadedReviews.length ? (
          ...
          <div className="mt-14 border-y border-line py-10">
            ...
            <p className="mt-2 text-sm leading-6 text-ink-muted">
              The first wallet-authorized review will appear here after a
              completed WrenPass transaction.
            </p>
          </div>
        )}
      </Container>
