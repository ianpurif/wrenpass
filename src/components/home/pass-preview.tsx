import { Bike, Check, Scissors } from "lucide-react";

export interface PassPreviewContent {
  businessName: string;
  icon: "bike" | "scissors";
  issueNumber: string;
  issued: number;
...
export function PassPreview({ campaign }: { campaign: PassPreviewContent }) {
  const BusinessIcon = campaign.icon === "bike" ? Bike : Scissors;
  const soldPercent = Math.min(100, Math.round((campaign.sold / campaign.issued) * 100));

  return (
    <article
      aria-label={`${campaign.businessName} sample WrenPass campaign`}
    ...
    >
      <div className="flex items-center justify-between border-b border-ink/15 px-5 py-4 sm:px-7">
        <div className="flex items-center gap-3">
          <BusinessIcon aria-hidden="true" className="size-4 text-coral-strong" strokeWidth={1.7} />
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="mt-10 grid grid-cols-2 border-y border-ink/15 py-5">
            <div>
              ...
              <p className="mt-2 text-2xl font-extrabold tracking-[-0.035em]">{campaign.payToday}</p>
            </div>
            <div className="border-l border-ink/15 pl-5">
              ...
              <p className="mt-2 text-2xl font-extrabold tracking-[-0.035em]">{campaign.serviceValue}</p>
            </div>
          </div>

          <div className="mt-6">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold">{campaign.sold} supported</span>
              <span className="text-ink-muted">{campaign.remaining} remaining</span>
            </div>
        ...
        <div className="relative flex border-t border-dashed border-ink/25 p-6 sm:border-l sm:border-t-0 sm:p-5">
          ...
          <div className="flex w-full items-end justify-between gap-6 sm:flex-col sm:items-stretch">
            <div>
              ...
              <p className="mt-2 text-sm font-bold">{campaign.validThrough}</p>
            </div>
            <div>
              <div
                aria-hidden="true"
      ...
      <div className="flex flex-col gap-2 border-t border-ink/15 px-5 py-4 text-[0.6rem] font-bold uppercase tracking-[0.14em] text-ink-muted sm:flex-row sm:items-center sm:justify-between sm:px-7">
        <span>{campaign.issued} passes issued</span>
